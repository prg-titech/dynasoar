...
__global__ void find_fish() {
  ...
  for (int i = 0; i < decltype(memory_allocator)::kN; ++i) {
    if (memory_allocator.is_allocated<Fish>(i)) {
...
__global__ void find_sharks() {
  ...
  num_sharks = 0;
  for (int i = 0; i < decltype(memory_allocator)::kN; ++i) {
    if (memory_allocator.is_allocated<Shark>(i)) {
      sharks[num_sharks++] = memory_allocator.get_obj<Shark>(i);
    }
  }
}
...
__global__ void find_cells() {
  ...
  int num_cells = 0;
  for (int i = 0; i < decltype(memory_allocator)::kN; ++i) {
    if (memory_allocator.is_allocated<Cell>(i)) {
      cells[num_cells++] = memory_allocator.get_obj<Cell>(i);
    }
  }
}
...
void initialize() {
  ...
  init_memory_system<<<GRID_SIZE_X*GRID_SIZE_Y/THREADS_PER_BLOCK + 1, THREADS_PER_BLOCK>>>();
  gpuErrchk(cudaDeviceSynchronize());

  create_cells<<<GRID_SIZE_X*GRID_SIZE_Y/THREADS_PER_BLOCK + 1, THREADS_PER_BLOCK>>>();
  gpuErrchk(cudaDeviceSynchronize());
  setup_cells<<<GRID_SIZE_X*GRID_SIZE_Y/THREADS_PER_BLOCK + 1, THREADS_PER_BLOCK>>>();
  gpuErrchk(cudaDeviceSynchronize());
