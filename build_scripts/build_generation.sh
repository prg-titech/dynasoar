...
while getopts "h?rda:s:m:x:" opt; do
    ...
    x)  args="${args} -DPARAM_SIZE=${OPTARG}"
        ;;
    r)  args="${args} -DOPTION_RENDER -lSDL2 example/generation/rendering.cu"
        ;;
...
args="${args} example/generation/dataset_loader.cu"
...
build_scripts/nvcc.sh ${args} example/generation/dynasoar/generation.cu -o bin/generation_dynasoar
